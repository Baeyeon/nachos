...
class Disk {
  public:
    ...
    int ComputeLatency(int newSector, bool writing);	 //计算寻道延迟
  ...
  private:
    ...
    int lastSector;			// The previous disk request   上次操作的扇区 用于计算寻道和等待时间
    int bufferInit;			// When the track buffer started  寻道相关
					// being loaded

    int TimeToSeek(int newSector, int *rotate); // time to get to the new track  寻道相关
    int ModuloDiff(int to, int from);        // # sectors between to and from寻道相关
    void UpdateLast(int newSector); //寻道相关
};
