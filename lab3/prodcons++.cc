...
{
    int num;
    slot *message = new slot(0,0);  //(thread_id,value)
...
//   page 182 of the textbook.
    ...
    for (num = 0; num < N_MESSG ; num++) {
      ...
      // Put the code for synchronization before  ring->Put(message) here.  //synchronization同步性
      // ...
      nempty->P();
      mutex->P();
...
Consumer(_int which)
{
    ...
    slot *message = new slot(0,0);  //由取出的message替代
